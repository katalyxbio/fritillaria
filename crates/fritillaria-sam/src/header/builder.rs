...
impl Builder {
    ...
    ///
    /// let header = sam::Header::builder()
    ///     .add_program("fritillaria-sam", Map::<Program>::default())
    ///     .build();
    ///
    ...
    /// assert_eq!(&comments[0], &b"fritillaria-sam"[..]);
    /// ```
...
mod tests {
    ...
    fn test_build() {
        let header = Builder::default()
            ...
            .add_comment("written by fritillaria-sam")
            .build();
        ...
        let comments = header.comments();
        assert_eq!(comments.len(), 1);
        assert_eq!(&comments[0], &b"written by fritillaria-sam"[..]);
    }
}
