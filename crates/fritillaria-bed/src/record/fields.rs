...
mod tests {
    ...
    #[test]
    fn test_parse_feature_start() -> io::Result<()> {
        assert_eq!(parse_feature_start(b"0")?, Position::MIN);

        assert!(matches!(
            parse_feature_start(b"-1"),
            Err(e) if e.kind() == io::ErrorKind::InvalidData
        ));

        assert!(matches!(
            parse_feature_start(Position::MAX.to_string().as_bytes()),
            Err(e) if e.kind() == io::ErrorKind::InvalidData
        ));
