...
mod tests {
    ...
    fn test_self() -> io::Result<()> {
        ...
        // Three writes with flushes between them, so the round trip crosses two
        // block boundaries. The payload is arbitrary; it just has to reassemble.
        writer.write_all(b"fritillaria")?;
        writer.flush()?;
        writer.write_all(b"-")?;
        writer.flush()?;
        ...
        assert_eq!(buf, b"fritillaria-bgzf");
