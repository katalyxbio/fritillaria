//! Proof that the vendored format readers compose on top of our reader.
//!
//! This is the load-bearing test for the whole integration strategy, and
//! vendoring made it more load-bearing rather than less. The vendored readers
//! are generic over `fritillaria_bgzf::io::BufRead` rather than over a concrete
//! type, so implementing that trait means every format — BAM here, but equally
//! BCF, `bgzip`ped VCF, and anything tabix-indexed — sits on our decompression
//! **without a single change to the vendored code**. That property is what
//! keeps a rebase onto a newer noodles mechanical; see `VENDORED.md`.
//!
...
//!
//! No feature gate any more: the traits used to live in an external crate, and
//! now they are vendored alongside, so the drop-in property is unconditional.
