...
use fritillaria_csi::BinningIndex;
use fritillaria_sam::{self as sam, alignment::RecordBuf, header::ReferenceSequences};
...
/// [`fritillaria_bgzf::io::MultithreadedReader`], etc., using [`Reader::from`].
///
...
/// ```
///
/// ### `fritillaria_bgzf::io::MultithreadedReader`
///
/// ```
/// # use std::{fs::File, io};
/// use fritillaria_bam as bam;
/// use fritillaria_bgzf as bgzf;
///
/// let decoder = bgzf::io::MultithreadedReader::new(io::empty());
/// let _reader = bam::io::Reader::from(decoder);
/// ```
pub struct Reader<R> {
    inner: R,
    ...
    R: Read,
{
    ...
    /// # use std::{fs::File, io};
    /// use fritillaria_bam as bam;
    /// use fritillaria_sam::alignment::RecordBuf;
    ///
    /// let mut reader = File::open("sample.bam").map(bam::io::Reader::new)?;
    /// let header = reader.read_header()?;
    ///
    ...
    R: bgzf::io::BufRead + bgzf::io::Seek,
{
    ...
    /// ```no_run
    /// # use std::fs::File;
    /// use fritillaria_bam::{self as bam, bai};
    ///
    /// let mut reader = File::open("sample.bam").map(bam::io::Reader::new)?;
    /// let header = reader.read_header()?;
    ///
