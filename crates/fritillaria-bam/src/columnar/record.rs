...
use crate::columnar::aux::{Array, Fields, Tag, Value, Values};
use crate::columnar::seq::{cigar_op_kind, cigar_op_len, decode_sequence, packed_len};
...
impl<'a> Record<'a> {
    ...
    /// Decoded aux fields.
    ///
    /// Lazy and zero-copy; see [`crate::columnar::aux`] for what the values borrow. Each
...
pub(crate) mod tests {
    ...
    pub(crate) fn build_record(name: &[u8], seq: &[u8], quals: Option<&[u8]>) -> Vec<u8> {
        ...
        for (i, base) in seq.iter().enumerate() {
            let nibble = crate::columnar::seq::BASES
                .iter()
                .position(|b| b == base)
                .unwrap() as u8;
            if i % 2 == 0 {
    ...
    fn handles_absent_qualities() {
        ...
        assert!(crate::columnar::seq::qualities_are_absent(
            record.qualities()
        ));
    }
    ...
    fn op(len: u32, kind: u8) -> u32 {
        let code = crate::columnar::seq::CIGAR_OPS
            .iter()
            .position(|&k| k == kind)
            .unwrap();
        (len << 4) | code as u32
